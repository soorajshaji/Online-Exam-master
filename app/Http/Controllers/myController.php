...
class myController extends Controller
{
    ...
    public function giveexam(\App\Relation\ExamDetail $exam_detail)
    {
      ...
      //$timestamp_1 = date(" Y-m-d H:i:s", $exam_detail->starts+1800);
     //dd($timestamp,$timestamp_1);
      // future
      //$exam_details = \App\Relation\ExamDetail::where('ends','<',time())->get();
      if($time <= $exam_detail->starts+1800){
        session()->flash('f',1);
         return redirect()->back();
       }
       $rs = \App\Relation\Result::where('student_id', \Auth::guard('students')->id())->where('exam_detail_id', $exam_detail->id)->first();
      return view('giveexam')->with(['e'=>$exam_detail->load('questions'),'rs'=>$rs]);
    }
    //
